...
class graphite::install::carbon (
  ...
  $carbon_pip_hack_source = "${python_pip_hack_source_path}/carbon-${version}-py${python_version}.egg-info"
  $carbon_pip_hack_target = "${carbon_pip_hack_target_path}/carbon-${version}-py${python_version}.egg-info"
  ...
  exec { 'carbon_cleanup_pip_egg_info_files':
    command => "find ${python_pip_hack_source_path} -name 'carbon-*.*.*-py${python_version}.egg-info' -not -name 'carbon-${version}-py${python_version}.egg-info' -exec rm {} \\;",
    onlyif  => "find ${python_pip_hack_source_path} -name 'carbon-*.*.*-py${python_version}.egg-info' -not -name 'carbon-${version}-py${python_version}.egg-info' | egrep '.*'",
    path    => $::path,
  }
}
